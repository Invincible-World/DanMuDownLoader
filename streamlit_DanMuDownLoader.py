...
DEFAULT_CONFIG = {
    ...
    "NAME_FORMAT": "[标题][集数]", # 输出文件名的格式模板：支持 [标题]、[集数]、[原] 三个占位符
...
def convert_xml_to_ass(xml_content):
    ...
    header = [
        "[Script Info]", "ScriptType: v4.00+", f"PlayResX: {PLAY_RES_X}", f"PlayResY: {PLAY_RES_Y}", "", 
        "[V4+ Styles]", 
    ...
    try:
        ...
        for d in root.findall('d'):
            ...
            if len(p) >= 4:
                ...
                # --- 新增逻辑：当停留时间为0，将顶部(5)和底部(4)转为滚动(1) ---
                mode = int(p[1])
                if CONFIG['STOP_DURATION'] <= 0 and mode in (4, 5):
                    mode = 1
                
                danmus.append({'start': float(p[0]), 'mode': mode, 'color': dec_to_ass_color(p[3]), 'text': text, 'w': w, 'h': int(CONFIG['ASS_FONT_SIZE'] * 1.2)})
...
with st.sidebar:
    ...
    with st.expander("🎨 命名与样式", expanded=True):
        ...
        with tag_col3: st.button("[原]", on_click=add_format_tag, args=("[原]",), use_container_width=True)
        with tag_col4: st.button("🗑️ 清空", on_click=clear_format, use_container_width=True)
    ...
    with st.expander("⏱️ 时间与显示", expanded=True):
        ...
        # 修改点：最小值改为 0，支持自动转换
        st.number_input("停留时长(秒)", 0, 20, key="cfg_STOP_DURATION", help="设为0时，顶底弹幕将自动转为滚动弹幕")
    ...
    if indices:
        ...
        if total_count > 1:
            ...
            if len(set(test_names)) < total_count:
                ...
                if "[集数]" not in current_fmt: current_fmt += "[集数]"
        ...
        for i, idx in enumerate(indices):
            ...
            clean_raw_title = re.sub(r'^[【\[].+?[\]】]\s*', '', raw_title)
            ep_tag = "" if is_movie_resource and total_count == 1 else f"E{idx+1:02d}"
            ...
            try:
                ...
                if content: st.session_state.download_files[f"{save_name}{suffix}"] = content
            except: pass
